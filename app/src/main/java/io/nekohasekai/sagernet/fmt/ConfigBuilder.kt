...
fun buildConfig(
    ...
    val isTransProxy = isRedir || isTproxy
    // "::" binds dual-stack (IPv4-mapped accepted), so transparent inbounds receive
    // both v4 and v6 redirected/tproxied traffic
    ...
    return MyOptions().apply {
        ...
        if (!forTest) {
            ...
            if (isRedir) {
                inbounds.add(Inbound_RedirectOptions().apply {
                    ...
                    // standalone sing-box 1.13+ removed inbound sniff/domain_strategy
                    if (!isTransProxy) {
                        sniff = needSniff
                        sniff_override_destination = needSniffOverride
                    }
            ...
            inbounds.add(Inbound_MixedOptions().apply {
                ...
                // VPN/proxy (libcore) still accepts legacy fields; standalone does not
                if (!isTransProxy) {
                    domain_strategy = genDomainStrategy(DataStore.resolveDestination)
                    sniff = needSniff
                    sniff_override_destination = needSniffOverride
        ...
        if (forTest) {
            ...
            // standalone (redir/tproxy): iptables REDIRECT rewrites the DNS destination
            ...
            // VPN: tun-in keeps inbound-level sniff (libcore), legacy pair works as-is
